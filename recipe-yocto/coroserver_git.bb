...
do_install() {
    install -d ${D}${bindir}
    install -m 0755 ${B}/coroserver ${D}${bindir}/coroserver

    install -d ${D}${sysconfdir}
    install -m 0644 ${S}/server-cert.pem ${D}${sysconfdir}/server-cert.pem
    install -m 0644 ${S}/server-key.pem ${D}${sysconfdir}/server-key.pem
}
