...
// TPM storage constants for certificates and keys
namespace TPMStorage
{
// TPM NV indices for certificates
namespace NVIndices
{
constexpr auto SELF_CA_CERT = "0x1500001";  // Self-signed CA certificate
constexpr auto SERVER_CERT = "0x1500002";   // Server entity certificate
constexpr auto CLIENT_CERT = "0x1500003";   // Client entity certificate
constexpr auto RECEIVED_CA_CERT =
    "0x1500004";                            // Received CA certificate from peer
constexpr auto CA_KEY_NV = "0x1500005";     // CA private key in NV
constexpr auto SERVER_KEY_NV = "0x1500006"; // Server private key in NV
constexpr auto CLIENT_KEY_NV = "0x1500007"; // Client private key in NV
} // namespace NVIndices

// TPM persistent handles for private keys
namespace PersistentHandles
{
constexpr auto CA_KEY = "0x81000001";     // CA private key (for signing)
constexpr auto SERVER_KEY = "0x81000002"; // Server entity private key
constexpr auto CLIENT_KEY = "0x81000003"; // Client entity private key
} // namespace PersistentHandles
} // namespace TPMStorage
    ...
    X509_NAME* issuer_name, int days_valid, bool is_ca)
{
    ...
    // Set notBefore to Unix epoch (January 1, 1970, 00:00:00 UTC) for maximum
    // validity This allows the certificate to be valid from the earliest
    // possible date
    ASN1_TIME* notBefore = X509_get_notBefore(cert.get());
    ASN1_TIME_set(notBefore,
                  0); // Unix epoch time (0 seconds since Jan 1, 1970)

    // Set notAfter to current time + days_valid for maximum forward validity
    // First set to current time (offset 0), then add days_valid
    ASN1_TIME* notAfter = X509_get_notAfter(cert.get());
    X509_gmtime_adj(notAfter, 0); // Set to current time first
    X509_gmtime_adj(notAfter, 60L * 60L * 24L * days_valid); // Add days_valid
...
{
    // Relaxed validity check - bare minimum for SSL handshake
    if (!cert)
    {
    ...
    // Only check that a public key exists - required for SSL handshake
