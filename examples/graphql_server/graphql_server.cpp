...
std::shared_ptr<GraphQLSchema> setupSchema(std::shared_ptr<DataStore> store)
{
    ...
    // Query: Get all users
    schema->addQuery("users", [store](const nlohmann::json& args,
    ...
    schema->addQuery("user", [store](const nlohmann::json& args,
                                     const nlohmann::json& context) {
        if (!args.contains("id"))
        {
    ...
    // Query: Get all posts
    schema->addQuery("posts", [store](const nlohmann::json& args,
                                      const nlohmann::json& context) {
        nlohmann::json result = nlohmann::json::array();
    ...
    schema->addQuery("postsByAuthor", [store](const nlohmann::json& args,
                                              const nlohmann::json& context) {
    ...
    schema->addMutation("createUser", [store](const nlohmann::json& args,
                                              const nlohmann::json& context) {
        if (!args.contains("name") || !args.contains("email"))
        {
            throw std::runtime_error("Missing required arguments: name, email");
        }
        ...
        return nlohmann::json{{"id", newUser.id},
                              {"name", newUser.name},
                              {"email", newUser.email},
                              {"age", newUser.age}};
    });
    ...
    schema->addMutation("createPost", [store](const nlohmann::json& args,
                                              const nlohmann::json& context) {
        ...
        newPost.authorId = args["authorId"].is_string()
                               ? std::stoi(args["authorId"].get<std::string>())
                               : args["authorId"].get<int>();

        store->posts.push_back(newPost);

        return nlohmann::json{{"id", newPost.id},
                              {"title", newPost.title},
    ...
    schema->addMutation("updateUser", [store](const nlohmann::json& args,
                                              const nlohmann::json& context) {
        if (!args.contains("id"))
        {
            throw std::runtime_error("Missing required argument: id");
...
int main(int argc, const char* argv[])
{
    try
    {
        ...
        auto executor = std::make_shared<GraphQLExecutor>(schema);
        ...
        router.add_post_handler(
            ...
            [executor](Request& req, const http_function& params)
                -> reactor::net::awaitable<Response> {
        ...
        LOG_INFO("GraphQL Server started on port {}", serverPort);
        LOG_INFO("Endpoints:");
